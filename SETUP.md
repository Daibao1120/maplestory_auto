...
- **`--edge-guard`（bat 已預設開啟）不會走出平台**：讀你在**小地圖上的真實位置**，
  到安全界就折返——用實際位置判斷、不靠盲數步數，所以不會掉下平台。需 opencv/mss
  （路線 B 已裝）；沒裝會自動退回下面的盲巡邏。
  - **`--edge-margin 6`**：安全範圍 = 起始 x ± 這麼多小地圖 px。**平台窄就調小**（4、3…）。
- **`--move-time 0.18` 每步走多遠**：按住方向鍵的秒數。**太短（<0.1）角色只會轉身、
  不會真的走**。（edge-guard 開著時它決定每步幅度，走多遠仍靠這個。）
- `--patrol-steps 1`：**只有 edge-guard 關閉/沒裝套件時**的盲巡邏才用到（往單邊幾步折返）。
- **攻擊方向**：**開場不會強制轉向**，沿用你角色當下的面向。想換邊就**按方向鍵
  （暫停）→ 轉到你要的方向 → 按 Ctrl 恢復**，它就跟著朝新方向打、移動後也轉回新方向。
  （`--face` 只是起始假設值，通常不用管。）
- `--interval-min 60 --interval-max 80` 多久巡邏挪一步：**定點約 90 秒攻擊才會失效**，
  所以在 90 秒前挪一次即可。**移動越少 = 掉平台機會越少**，時間夠就盡量拉長。
- **`--no-refocus`（bat 已預設開啟）打字不被干擾**：你切到別的視窗（打字、查資料）時
  它**自動暫停、不搶焦點、不送任何鍵進你的視窗**；切回楓之谷會**自動恢復**攻擊。
  想改成「別的視窗彈出時自動搶回楓之谷繼續打」就把 `--no-refocus` 拿掉。
- `--no-move` 完全不動只打
