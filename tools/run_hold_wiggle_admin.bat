...
REM   --interval-min 90       shortest gap between patrol steps (seconds).
REM   --interval-max 100      Attacks go stale after ~90s standing still, so it
REM                           only repositions after ~90s. Fewer moves = fewer
...
"C:\Users\ibuzz\anaconda3\envs\linebot_RAG\python.exe" tools\hold_and_wiggle.py --key ctrl --attack-interval 0.10 --move-time 0.15 --edge-guard --edge-margin 6 --face left --interval-min 90 --interval-max 100 --no-refocus
